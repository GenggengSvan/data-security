...
void print_tree(Node* node, int depth) {
    ...
    string type = (node->type == 0) ? "LeafNode" : "InternalNode";
    cout << setw(depth*4) << "" << "[" << type << "]" << endl;

    if (node->type == 0) { // LeafNode
        LeafNode* leaf = static_cast<LeafNode*>(node);
        for (int i = 0; i < leaf->cipher.size(); i++) {
            cout << setw((depth+1)*4) << "" << i << ": " << leaf->cipher[i] << " (" << leaf->encoding[i] << ")" << endl;
        }
    } else { // InternalNode
        InternalNode* internal = static_cast<InternalNode*>(node);
        for (int i = 0; i < internal->child.size(); i++) {
            print_tree(internal->child[i], depth+1);
        }
    }
}
